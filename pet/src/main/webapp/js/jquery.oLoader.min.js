...
(function(e) {
	...
	e.ownage.Loader = function(t, n, r, i) {
		...
		s.init = function() {
			...
			if (typeof n === "string") {
				...
				e.extend(e.ownage.Loader.objectOptions[s.objectIndex],
						e.ownage.Loader.defaultOptions, n);
		...
		s.adjustLoader = function() {
			...
			if (t.left == s.lastPosLeft && t.top == s.lastPosTop
					&& n == s.lastWidth && r == s.lastHeight
		...
		s.setCheckInterval = function() {
			e("#" + s.overlaySel).css({
				background : s.params.backgroundColor
			});
		...
		s.show = function() {
			if (e("#" + s.canvasSel).length > 0)
				return;
			if (s.context.parent().hasClass("oloader_canvas")
					|| s.context.hasClass("oloader_canvas"))
				return;
			e("body")
					.append(
							"<div id='"
									+ s.canvasSel
									+ "' class='oloader_canvas' style='padding:0px;margin:0px;display:none;overflow:hidden'></div>");
			if (s.params.modal) {
				e("#" + s.canvasSel).append(
						"<div id='" + s.overlaySel + "'></div>");
			...
			e("#" + s.canvasSel).fadeTo(s.params.fadeOutTime, 0, function() {
				if (!t) {
					...
					if (typeof s.params.complete === "function") {
						if (s.params.successData)
							sucData = s.params.successData;
		...
		s.setStyle = function() {
			...
			case 4:
				e = "<img src='"
						...
						+ "' id='"
						+ s.loaderSel
						+ "' style='box-shadow:0px 0px 20px #000;position:absolute;padding:"
						+ s.params.imagePadding + "px;background:"
						+ s.params.imageBgColor + "' alt='Loading...' />";
				break;
			case 3:
				e = "<img src='"
						+ s.params.image
						+ "' id='"
						+ s.loaderSel
						+ "' style='box-shadow:0px 0px 20px #222;border-radius:1000px;position:absolute;padding:"
						+ s.params.imagePadding + "px;background:"
						+ s.params.imageBgColor + "' alt='Loading...' />";
				break;
			case 2:
				e = "<div id='"
						+ s.loaderSel
						+ "' style='position:absolute;width:100%;padding-top:10px;padding-bottom:10px;";
				e += "text-align:center;background-color:"
						+ s.params.imageBgColor + "'><img src='"
						+ s.params.image + "' alt='Loading...' /></div>";
				break;
			case 0:
				e = "<img src='" + s.params.image + "' id='" + s.loaderSel
						+ "' style='position:absolute' alt='Loading...' />";
				break;
			case 1:
			default:
				e = "<img src='"
						...
						+ "' id='"
						+ s.loaderSel
						+ "' style='box-shadow:1px 1px #000;border-radius:1000px;position:absolute;padding:"
						+ s.params.imagePadding + "px;background:"
						+ s.params.imageBgColor + "' alt='Loading...' />";
				break
			}
		...
		s.executeEffect = function(t) {
			...
			case "door":
				...
				myData = {
					backgroundColor : s.params.backgroundColor,
					fadeLevel : s.params.fadeLevel
				};
			...
			case "slide":
				...
				if (t) {
					e("#" + s.overlaySel).css("left",
							-e("#" + s.overlaySel).width())
				} else {
					i = e("#" + s.overlaySel).width()
				}
				e("#" + s.overlaySel).animate({
					left : i
				}, t ? s.params.fadeInTime : s.params.fadeOutTime);
				break
			}
		};
		s.preloadImages = function() {
			var t = [ "../resource/image/loader1.gif",
					"../resource/image/loader2.gif",
					"../resource/image/loader3.gif",
					"../resource/image/loader4.gif",
					"../resource/image/loader5.gif",
					"../resource/image/loader6.gif",
					"../resource/image/loader7.gif" ];
			var n = "";
			...
			e("body")
					.append(
							"<div class='oloader_image_preload' style='position:absolute;left:-5000px;top:-5000px;'>"
									+ n + "</div>");
	...
	e.ownage.PageLoader = function(t) {
					...
					zIndex : "1000"
				}).css(
						{
							marginTop : -(n.options.progressBarHeight / 2)
									- e("#ownage_page_loader_text").height()
						});
		...
		n.additionalImages = function() {
			if (n.options.images.length == 0)
				return;
			e("body")
					.append(
							"<div style='position:absolute;left:-10000px;top:-10000px;display:none;' id='ownage_page_loader_addImages'></div>");
			for (i = 0; i < n.options.images.length; i++) {
				e("#ownage_page_loader_addImages").append(
						"<img src='" + n.options.images[i] + "' />")
			}
		};
		n.updateProgressBar = function() {
			var t = parseInt(n.loaded / n.total * 100);
			if (t == n.lastPercentage)
				return;
			n.lastPercentage = t;
			e("#ownage_page_loader_text").html(t + "%");
			if (!n.options.ownStyle) {
				e("#ownage_page_loader_text").css({
					marginLeft : -(e("#ownage_page_loader_text").width() / 2)
				})
			}
			e("#ownage_page_loader").stop().animate({
				width : t + "%"
			}, 100);
